...
def initialize_chatbot(retriever, web_retriever, session_id):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a financial advisor called Investibot. You give precise recommendations. When requested about financial information, refer to 'retriever_financial_search'. For dates, use retriever_date_search. For all else, Trading_Research."),
    ...
    retriever_new_search = create_retriever_tool(
        retriever=web_retriever,
        name="retriever_date_search",
        description="Use this tool when searching updated Date information."
    )
    ...
    tools = [researcher, retriever_pinecone_search, retriever_new_search]  # List of tools available to the chatbot.
...
async def on_chat_start():
    ...
    url = "https://www.timeanddate.com/"  # URL for web-based retriever initialization.
...
Upstash is used for conversation memory based on session id. (Free & Serverless. Create a free account) 
...
Running the Application: The if _name_ == '_main_': block runs the Chainlit application.
